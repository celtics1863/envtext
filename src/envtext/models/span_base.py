...
class SpanBase:
    def align_config(self):
        ...
        if hasattr(self.config, "word2vec"): #使用word2vec增强bert输出得特征，解决边界问题
            self.word2vec = True
    ...
    def compute_metrics(self,eval_pred):
        ...
        print(preds.shape,logits.shape,labels.shape)
        ...
        for b in range(B):
            for n in range(N):
                ...
                for p in preds[b]:
                    ...
                    if p[0] == n:
                        pred.add((p[0],p[1],p[2]))

            
                start = 0
                end = 0
                for l in range(L):
                    if labels[b,n,l,0] == -100:
                        break
